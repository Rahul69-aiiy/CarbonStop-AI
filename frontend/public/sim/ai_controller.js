...
const ML_API_URL = "https://carbonstop-ai.onrender.com";
...
class AIController {
  constructor() {
    ...
    this.log(
      "🧠 AI Traffic Optimizer v3.0 — Fairness-Aware + Ambulance Priority",
    );
  ...
  tick() {
    ...
    dirs.forEach((d) => {
      ...
      laneState[d].priority =
        FAIR_CONFIG.alpha * normQ + FAIR_CONFIG.beta * normRed;
    });
    ...
    if (comparisonData.isRunningBaseline) {
      ...
      if (
        comparisonData.baselinePhaseTimer >=
        comparisonData.baselinePhaseInterval
      ) {
    ...
    dirs.forEach((d) => {
      ...
      if (row) {
        row.classList.toggle(
          "starvation-warn",
          laneState[d].redDuration > FAIR_CONFIG.starvationThreshold * 0.8,
        );
      }
    });
  ...
  updateCharts() {
    ...
    const avgWait =
      waitingVehicles.length > 0
        ? waitingVehicles.reduce(
            (s, v) => s + (Date.now() - v.waitStart) / 1000,
            0,
          ) / waitingVehicles.length
        : 0;

    chartData.waitTime.push(parseFloat(avgWait.toFixed(1)));
    if (chartData.waitTime.length > chartData.maxPoints)
      chartData.waitTime.shift();
    ...
    const co2Rate =
      idleCount * EMISSIONS.IDLE.co2 + movingCount * EMISSIONS.MOVING.co2;
    chartData.co2Rate.push(parseFloat(co2Rate.toFixed(1)));
    if (chartData.co2Rate.length > chartData.maxPoints)
      chartData.co2Rate.shift();
  ...
  async checkBackendStatus() {
    ...
    try {
      ...
      if (stEl) {
        stEl.innerText = data.q_table_states ?? "—";
      }
    } catch {
      if (dot) {
        dot.className = "backend-dot disconnected";
      }
      if (label) {
  ...
  updateMLOutputPanel(data) {
    ...
    if (data.ambulance_override) {
      if (actionEl) {
        actionEl.innerText = "🚑 AMBULANCE";
        actionEl.style.color = "#f87171";
      }
      if (badgeEl) {
        badgeEl.innerText = "🚨 OVERRIDE";
  ...
  async update() {
    ...
    // During baseline, skip AI logic entirely
    ...
    // ─── AMBULANCE OVERRIDE (Only acts if AI is enabled) ───
    if (ambulanceActive && isAIEnabled) {
      const ambDir = ambulanceActive.direction;
      if (currentPhase !== ambDir) {
        this.log(
          `🚑 AMBULANCE PRIORITY: ${ambDir} — forcing immediate green override.`,
        );
    ...
    // ─── RULE-BASED TIMERS (Only acts if AI is enabled) ───
    if (isAIEnabled) {
      if (
        currentPhase === "NS" &&
        maxRedEW > FAIR_CONFIG.starvationThreshold &&
        elapsed >= FAIR_CONFIG.minGreen
      ) {
        this.starvationEvents++;
        this.log(
          `🚨 STARVATION! EW red for ${Math.floor(maxRedEW)}s — forcing green.`,
        );
        await this.performSwitch();
        return;
      }
      if (
        currentPhase === "EW" &&
        maxRedNS > FAIR_CONFIG.starvationThreshold &&
        elapsed >= FAIR_CONFIG.minGreen
      ) {
        this.starvationEvents++;
        this.log(
          `🚨 STARVATION! NS red for ${Math.floor(maxRedNS)}s — forcing green.`,
        );
        await this.performSwitch();
        return;
      }
    ...
    // ─── Q-Learning Decision (Always fetch to update ML panel UI, but only act if AI enabled) ───
    ...
    try {
      ...
      const payload = {
        queue_NS: Math.min(20, q.NS),
        queue_EW: Math.min(20, q.EW),
        red_NS: Math.min(60, Math.floor(maxRedNS)),
        red_EW: Math.min(60, Math.floor(maxRedEW)),
        phase: apiPhase,
      ...
      if (data.action === "switch_phase") {
        ...
        data.action === "extend_green" ||
        data.action === "keep_green"
      ) {
        action = "KEEP";
      }
      ...
      if (data.ambulance_override) {
        // Ambulance override logging
        if (isAIEnabled) this.log(`🚑 ML: AMBULANCE PRIORITY → ${data.action}`);
        if (action === "SWITCH" && isAIEnabled) await this.performSwitch();
      ...
      } else if (this.episodeSteps % 4 === 0 && isAIEnabled) {
        this.log(
          `🌐 ML Backend: KEEP ${currentPhase} | ${data.explanation} (CO₂ ×${data.carbon_intensity.toFixed(2)})`,
        );
      }
    } catch (e) {
      ...
      if (this.prevStateKey !== null && this.prevAction !== null) {
        ...
        const bestFuture = Math.max(
          getQValue(stateKey, "KEEP"),
          getQValue(stateKey, "SWITCH"),
        );
        const newQ =
          oldQ + Q_CONFIG.alpha * (reward + Q_CONFIG.gamma * bestFuture - oldQ);
      ...
      // Decay epsilon
      currentEpsilon = Math.max(
        Q_CONFIG.epsilonMin,
        currentEpsilon * Q_CONFIG.epsilonDecay,
      );
      ...
      if (this.rewardEl)
        this.rewardEl.innerText = Math.max(
          0,
          Math.floor(1000 + this.totalReward),
        );

      // Force Dashboard ML UI to show local fallback state!
      this.updateMLOutputPanel({
        action: action === "SWITCH" ? "switch_phase" : "keep_green",
        carbon_intensity: 1.0, // fallback
        explanation: "Fallback: Local Q-Table Decision",
      ...
      if (action === "SWITCH") {
        ...
        if (isAIEnabled) {
          this.log(
            `⚡ LOCAL Agent: SWITCH → ${target} | P(NS)=${prioNS.toFixed(2)} P(EW)=${prioEW.toFixed(2)} ε=${currentEpsilon.toFixed(3)}`,
          );
          await this.performSwitch();
        }
      } else {
        // Periodically log KEEP decisions
        if (this.episodeSteps % 4 === 0 && isAIEnabled) {
          this.log(
            `✅ LOCAL Agent: KEEP ${currentPhase} green | Q=${q.NS}/${q.EW} | ε=${currentEpsilon.toFixed(3)}`,
          );
        }
      }
    }
  ...
  async performSwitch() {
    ...
    if (signals.NS === SIGNAL.GREEN) {
      ...
      console.log("🟡 AI: NS signal to YELLOW");
      this.log("🟡 NS → YELLOW");
      updateSignalLights();
      await sleep(2000);
      signals.NS = SIGNAL.RED;
      signals.EW = SIGNAL.GREEN;
      console.log("🔴🟢 AI: NS RED, EW GREEN");
      this.log("🔴 NS: RED | 🟢 EW: GREEN");
    } else {
      signals.EW = SIGNAL.YELLOW;
      console.log("🟡 AI: EW signal to YELLOW");
      this.log("🟡 EW → YELLOW");
      updateSignalLights();
      await sleep(2000);
      signals.EW = SIGNAL.RED;
      signals.NS = SIGNAL.GREEN;
      console.log("🔴🟢 AI: EW RED, NS GREEN");
  ...
  updateBaselineProgress() {
    ...
    if (progBar)
      progBar.style.width =
        (comparisonData.baselineTimer / comparisonData.baselineDuration) * 100 +
        "%";
    if (progText)
      progText.innerText = `${Math.floor(comparisonData.baselineTimer)}/${comparisonData.baselineDuration}s`;
  }

  endBaseline() {
    comparisonData.isRunningBaseline = false;
    const avgWait =
      metrics.throughput > 0 ? metrics.totalWait / metrics.throughput : 0;
    const maxRedAll = Math.max(
      laneState.N.redDuration,
      laneState.S.redDuration,
      laneState.E.redDuration,
      laneState.W.redDuration,
    );
    ...
    this.log(
      `📏 BASELINE complete! AvgWait=${comparisonData.baseline.avgWait}s CO₂=${comparisonData.baseline.co2}g`,
    );
  ...
  recordAIMetrics() {
    ...
    this.log(
      `🤖 AI metrics recorded! AvgWait=${comparisonData.ai.avgWait}s CO₂=${comparisonData.ai.co2}g`,
    );
  ...
  updateComparisonUI() {
    ...
    const setComparison = (id, baseVal, aiVal, unit, reverse) => {
      ...
      if (impEl) {
        const pct =
          baseVal > 0 ? (((baseVal - aiVal) / baseVal) * 100).toFixed(0) : 0;
  ...
  updateScalability() {
    ...
    const hourlyPerIntersection =
      emissionsTotal.co2 > 0
        ? (emissionsTotal.co2 * 0.17 * 3600) /
    ...
    const daily = (
      (hourlyPerIntersection * intersections * 24) /
      1000000
    ).toFixed(1); // tons
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (btnBaseline) {
    btnBaseline.onclick = () => {
      console.log("📏 BASELINE button clicked: Starting baseline comparison");
      if (!isRunning) {
...
function dispatchAmbulance(direction) {
  ...
  const timerInterval = setInterval(() => {
    ...
    if (timerEl)
      timerEl.innerText = left > 0 ? `Auto-clear: ${left.toFixed(0)}s` : "";
  }, 300);
  ...
  ai.log(
    `🚑 AMBULANCE DISPATCHED → ${direction} corridor! Immediate green priority.`,
  );
...
function clearAmbulance() {
  ...
  if (ambulanceClearTimer) {
    clearTimeout(ambulanceClearTimer);
    ambulanceClearTimer = null;
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (btnAmbNS)
    btnAmbNS.onclick = () => {
      console.log("🚑 Dispatch NS button clicked");
      dispatchAmbulance("NS");
    };
  if (btnAmbEW)
    btnAmbEW.onclick = () => {
      console.log("🚑 Dispatch EW button clicked");
      dispatchAmbulance("EW");
    };
  if (btnClear)
    btnClear.onclick = () => {
      console.log("✕ Clear Ambulance button clicked");
      clearAmbulance();
