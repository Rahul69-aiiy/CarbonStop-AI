...
const TRAFFIC_PATTERNS = {
  ...
  rushNS: { N: 0.4, S: 0.35, E: 0.15, W: 0.1, rate: 0.03 },
  rushEW: { N: 0.12, S: 0.13, E: 0.4, W: 0.35, rate: 0.03 },
...
class Vehicle {
  ...
  update(others) {
    ...
    const mySignal =
      this.origin === "N" || this.origin === "S" ? signals.NS : signals.EW;
    let canGo = true;
    ...
    const followLimit = this.isAmbulance ? 75 : 50;
    if (closestDist < followLimit) canGo = false;

    // Speed calculation
    const maxSpd = this.isAmbulance ? MAX_SPEED * 1.5 : MAX_SPEED;
    if (!canGo) {
      // Decelerate — ambulances brake harder to avoid collision at high speeds
      const decelRate = this.isAmbulance ? DECEL * 1.5 : DECEL;
      this.speed = Math.max(0, this.speed - decelRate * timeScale);
...
function updateMetrics() {
  document.getElementById("m-vehicles").innerText = vehicles.filter(
    (v) => v.active,
  ).length;
  const avgWait =
    metrics.throughput > 0
      ? (metrics.totalWait / metrics.throughput).toFixed(1)
      : "0.0";
  document.getElementById("m-wait").innerText = avgWait + "s";
  const elapsed = (Date.now() - metrics.startTime) / 60000;
  document.getElementById("m-throughput").innerText = (
    cleared / Math.max(0.01, elapsed)
  ).toFixed(0);
...
function updateSignalLights() {
  ...
  if (nsBtn) {
    ...
    nsBtn.className =
      "btn btn-signal" + (signals.NS === SIGNAL.GREEN ? " active-signal" : "");
  }
  if (ewBtn) {
    ewBtn.innerText = signals.EW === SIGNAL.GREEN ? "Force RED" : "Force GREEN";
    ewBtn.className =
      "btn btn-signal" + (signals.EW === SIGNAL.GREEN ? " active-signal" : "");
  }
}
...
function drawBackground() {
  ...
  ctx.fillRect(
    cx + hw + 5,
    cy + hw + 5,
    canvas.width - cx - hw - 15,
    canvas.height - cy - hw - 15,
  );
}
...
function drawRoads() {
  ...
  ctx.stroke();
  ctx.beginPath();
  ctx.moveTo(cx - hw, cy + hw);
  ctx.lineTo(cx - hw, canvas.height);
  ctx.stroke();
  ctx.beginPath();
  ctx.moveTo(cx + hw, 0);
  ctx.lineTo(cx + hw, cy - hw);
  ...
  ctx.beginPath();
  ctx.moveTo(cx - hw + 4, cy - stopLineOffset);
  ctx.lineTo(cx, cy - stopLineOffset);
  ctx.stroke();
  // South stop
  ctx.beginPath();
  ctx.moveTo(cx, cy + stopLineOffset);
  ctx.lineTo(cx + hw - 4, cy + stopLineOffset);
  ctx.stroke();
  // East stop
  ctx.beginPath();
  ctx.moveTo(cx + stopLineOffset, cy - hw + 4);
  ctx.lineTo(cx + stopLineOffset, cy);
  ctx.stroke();
  // West stop
  ctx.beginPath();
  ctx.moveTo(cx - stopLineOffset, cy);
  ctx.lineTo(cx - stopLineOffset, cy + hw - 4);
  ctx.stroke();
...
function loop() {
  ...
  if (isRunning) {
    ...
    const pattern =
      TRAFFIC_PATTERNS[currentPattern] || TRAFFIC_PATTERNS.balanced;
...
btnPause.onclick = () => {
  console.log(
    isRunning
      ? "⏸ PAUSE button clicked: Pausing simulation"
      : "▶ RESUME button clicked: Resuming simulation",
  );
  isRunning = !isRunning;
  if (isRunning) {
...
btnReset.onclick = () => {
  console.log("↺ RESET button clicked: Resetting simulation");
  isRunning = false;
  isAIEnabled = false;
  vehicles = [];
  ...
  // Reset emissions and AI state completely
  if (typeof emissionsTotal !== "undefined") {
    emissionsTotal.co2 = 0;
    emissionsTotal.fuel = 0;
    emissionsTotal.nox = 0;
  }
  if (typeof laneState !== "undefined") {
    ["N", "S", "E", "W"].forEach((d) => {
      laneState[d].priority = 0.01;
      laneState[d].redDuration = 0;
    });
  }
  if (window.ai) {
    window.ai.totalReward = 0;
    window.ai.episodeSteps = 0;
    window.ai.starvationEvents = 0;
    window.ai.peakEmission = 0;
    window.ai.log("↺ Simulation Reset.");
  }
};
...
btnAI.onclick = () => {
  ...
  console.log(
    isAIEnabled
      ? "🤖 AI MODE button clicked: AI enabled"
      : "🤖 AI MODE button clicked: AI disabled",
  );
  btnAI.classList.toggle("active", isAIEnabled);
  btnAI.innerText = isAIEnabled ? "🤖 AI: ON" : "🤖 AI MODE";
  document.getElementById("ai-mode-badge").innerText = isAIEnabled
    ? "AI"
    : "MANUAL";
  document.getElementById("ctrl-mode-note").innerText = isAIEnabled
    ? "AI"
    : "MANUAL";
};

// Prevent spamming manual clicks during transition
let isTransitioning = false;

btnNS.onclick = () => {
  if (isAIEnabled || isTransitioning) return;
  if (signals.NS === SIGNAL.GREEN) return; // Already green

  console.log("🔴 Force GREEN button clicked for NS: Initiating transition");
  isTransitioning = true;
  if (window.ai)
    ai.log("✋ MANUAL: Forcing NS to GREEN (Yellow transition...)");
  ...
  setTimeout(() => {
    ...
    console.log("🟢 NS signal now GREEN");
    if (window.ai) ai.log("🟢 MANUAL: NS is now GREEN");
  }, 1500);
};

btnEW.onclick = () => {
  if (isAIEnabled || isTransitioning) return;
  if (signals.EW === SIGNAL.GREEN) return; // Already green

  console.log("🔴 Force GREEN button clicked for EW: Initiating transition");
  isTransitioning = true;
  if (window.ai)
    ai.log("✋ MANUAL: Forcing EW to GREEN (Yellow transition...)");
  ...
  setTimeout(() => {
    ...
    updateSignalLights();
    isTransitioning = false;
    console.log("🟢 EW signal now GREEN");
    if (window.ai) ai.log("🟢 MANUAL: EW is now GREEN");
  }, 1500);
};

document.getElementById("sim-speed").oninput = (e) => {
  timeScale = parseFloat(e.target.value);
  console.log(`Speed changed to ${timeScale}×`);
...
if (patternSelect) {
  patternSelect.onchange = (e) => {
    ...
    console.log(`Traffic pattern changed to: ${currentPattern}`);
  };
}

document.getElementById("btn-inject").onclick = () => {
  const activeBtn = document.querySelector(".dir-btn.active");
  const activeDir = activeBtn ? activeBtn.dataset.dir : "N";
  const count = parseInt(document.getElementById("inject-count").value) || 1;

  // Automatic start if paused so user immediately sees the cars roll in
  if (!isRunning) {
    document.getElementById("btn-start").click();
  }

  // UI feedback so user knows it registered immediately
  const btnInject = document.getElementById("btn-inject");
  const oldText = btnInject.innerText;
  btnInject.innerText = "Injecting...";
  btnInject.disabled = true;

  let delayMs = 0;
  const injectStaggered = (dir) => {
    for (let i = 0; i < count; i++) {
      setTimeout(() => spawnVehicle(dir), delayMs);
      delayMs += 600; // Stagger cars by 600ms so they don't overlap
    }
  };

  if (activeDir === "ALL") {
    ["N", "S", "E", "W"].forEach((d) => injectStaggered(d));
  } else {
    injectStaggered(activeDir);
  }

  // Restore button after finished
  setTimeout(() => {
    btnInject.innerText = oldText;
